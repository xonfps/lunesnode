...
case class SignedRegistryRequest(
  ...
  def toTx: Either[ValidationError, RegistryTransaction] =
    for {
      _sender <- PublicKeyAccount.fromBase58String(senderPublicKey)
      _assetId <- parseBase58ToOption(
        assetId.filter(_.length > 0),
        "invalid.assetId",
        AssetIdStringLength
      )
      _feeAssetId <- parseBase58ToOption(
        feeAssetId.filter(_.length > 0),
        "invalid.feeAssetId",
        AssetIdStringLength
      )
      _signature <- parseBase58(
        signature,
        "invalid.signature",
        SignatureStringLength
      )
      _userdata <- parseBase58(
        userdata.filter(_.length > 0),
        "invalid.userdata",
        RegistryTransaction.MaxUserdataLength
      )
      _account <- AddressOrAlias.fromString(recipient)
      t <- RegistryTransaction.create(
        _assetId,
        _sender,
        _account,
        amount,
